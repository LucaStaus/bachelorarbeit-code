...
class GCC<V, E>(graph: SimpleGraph<V, E>) : Algo<V, E>(graph) {

    var lastUpperBound = Triple<Set<V>, Int, Float>(HashSet(), 0, 0f)
        private set

    var checkedSubsets = 0
        private set
    var sufficientVertices = HashSet<V>(vertCount)
        private set

    private var initialSufficientIndices: MutableSet<Int> = HashSet(vertCount)

    override fun preparations() {
        createVertexLists()
        calcNeighborhoods()
        createDistanceMatrix()
        calcSufficientVertexSet()
    }

    private fun calcSufficientVertexSet() {
        val result = initialSufficientIndices
        val eliminated = HashSet<Int>(vertCount)
        for (vi in vertexList.indices) {
            if (vi in eliminated) continue
            result.add(vi)
            // find all vertices that are dominated by vi
            val vertSet = HashSet<Int>(vertCount)
            // start by finding vertices that fulfill the first property
            for (neigh in neighborhoods[vi]) {
                vertSet.addAll(neighborhoods[neigh])
                if (neigh in eliminated) continue
                // check first property
                if (checkIfSubset(neighborhoods[neigh], vi, neighborhoods[vi])) {
                    result.remove(neigh)
                    eliminated.add(neigh)
                }
            }
            vertSet.removeAll(neighborhoods[vi])
            vertSet.remove(vi)
            // now find vertices that fulfill the second property
            for (vin in vertSet) {
                if (vin in eliminated) continue
                // check second property
                if (neighborhoods[vin].size == neighborhoods[vi].size && neighborhoods[vin] == neighborhoods[vi]) {
                    result.remove(vin)
                    eliminated.add(vin)
                }
            }
        }
        sufficientVertices = HashSet(initialSufficientIndices.size)
        for (vi in initialSufficientIndices) {
            sufficientVertices.add(vertexList[vi])
        }
    }

    override fun calcResult(): Triple<Set<V>, Int, Float> {
        return if (k < initialSufficientIndices.size) {
            checkEverySubSet()
        } else {
            val result = HashSet<V>(k)
            for (vi in initialSufficientIndices) {
                result.add(vertexList[vi])
            }
            ...
            Triple(result, vertCount - k, 1f)
        }
    }

    private fun checkEverySubSet() : Triple<Set<V>, Int, Float> {
        // resetting information variables
        checkedSubsets = 0

        // UPPER BOUND
        // variables for keeping track of the best result + calculating Upper Bound
        var (bestCentrality, bestIndices) = calcUpperBoundGreedy()
        val set = HashSet<V>(k)
        bestIndices.forEach { set.add(vertexList[it]) }
        lastUpperBound = Triple(set, bestCentrality, (vertCount - k).toFloat() / bestCentrality.toFloat())
        ...
        // INITIAL SORTING
        val sortDiam = ((diameter * vertCount).toFloat() / (edgeCount * 3).toFloat() + 1).roundToInt()
        for (vi in initialSufficientIndices) {
            centImprovements[nextI][vi] = distanceMatrix[vi].sumBy { if (it <= sortDiam) 1 else 0 }
        ...
        if (k == 1) {
            ...
            while (vertOrder[0].isNotEmpty()) {
                ...
                checkedSubsets++
                if (curCentrality < bestCentrality) {
                ...
                when (nextAction) {
                    ...
                    NextAction.STAY -> {
                        if (nextI > 0) {
                            ...
                            if (centralities[nextI - 1] - centImprovementSums[nextI] >= bestCentrality) {
                                ...
                                if (curCentrality - lb > bestCentrality) {
                    ...
                    NextAction.DOWN -> {
                        ...
                        if (spacesLeft > 1) {
                            ...
                            for (vi in topImprovementVerts[nextI - 1]) {
                                if (centImprovements[nextI - 1][vi] <= lb) break
